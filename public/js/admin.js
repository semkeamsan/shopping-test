...
var Layout = function () {
    ...
    Charts = function () {
        ...
        return window.Chart && o(Chart, (e = {
                ...
                Chart.scaleService.updateScaleDefaults("Project", {
                    gridLines: {
                        drawBorder: !1,
                        drawOnChartArea: !1,
                        drawTicks: !1
        ...
        n.length && n.each(function(){
            (e = $(this),
                ...
                t = new Chart(e, {
                    type: "pie",
                    data: {
                        labels: e.data('read') ? Object.keys(e.data('read'))  : ["Danger", "Warning", "Success", "Primary", "Info"],
                        datasets: [{
                            data: e.data('read') ? Object.values(e.data('read')) : [a(), a(), a(), a(), a()],
                            backgroundColor: [Charts.colors.theme.success, Charts.colors.theme.info, Charts.colors.theme.secondary, Charts.colors.theme.primary, Charts.colors.theme.warning],
                            label:  e.data('label')  ?? "Dataset 1"
                        }]
                    },
                    options: {
                        responsive: !0,
    ...
    BarsChart = (SalesChart = function () {
            ...
            t.length && (e = t,
                a = new Chart(e, {
                    ...
                    data: {
                        labels: e.data('read') ? Object.keys(e.data('read')) : ["May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"] ,
                        datasets: [{
                            label: e.data('label')  ?? "Performance",
                            data: e.data('read') ? Object.values(e.data('read')) :  [0, 20, 10, 30, 15, 40, 20, 60, 60]
                        }]
                    }
                }),
                e.data("chart", a))
...
var DatatableBasic = function () {
    function t ($t){
        var e = $t || $("table#datatable-basic");

        e.length && e.each(function(){
            $(this).on("init.dt", function () {
                $("div.dataTables_length select").attr('data-minimum-results-for-search','Infinity').select2({}).parent().css({width : 120});
                $("#datatable-basic_filter").find('input').attr('placeholder',$(this).data('text-search'));
                $("div#datatable-basic_filter").addClass('float-right');
            }).DataTable({
                keys: !0,
                ordering: false,
                responsive: true,
                ...
                autoWidth: true,
                scrollCollapse: true,
                fixedColumns:  $(this).data('fixed-left') || $(this).data('fixed-right') ? {
                    leftColumns: $(this).data('fixed-left')??0,
                    rightColumns: $(this).data('fixed-right')??0
                } : false,
                language: window.datatable_lang ?? {
                    paginate: {
                ...
                //stateSave: true,
                lengthMenu: [[10, 25, 50,100, -1], [10, 25, 50,100,  ($(this).data('text-all') ?? "All")]],
                iDisplayLength : $(this).data('display')??25,
                paging : $(this).data('paging')??false,
                info : $(this).data('paging')??false,
            });

            $(this).find(`select`).select2({});
            if(!$(this).data('fixed-left') || !$(this).data('fixed-right')){
                $('table.dataTable, .dataTables_scrollHeadInner').css({width : '100%'});
                $(this).css({width : '100%'}).parent().css({width : '100%'});
            }
        });
    }
t();
    if($(`table[data-toggle="datatable-ajax"]`).length){

        $.get(location.href).done((res) => {
            if (res) {
                var $table = $(res);
                var $p = $(`table[data-toggle="datatable-ajax"]`).parents('.table-responsive');
                    $("table#datatable-basic").remove();
                    $p.html( $table);
                    t($table);
            }
        })
            ...
            e.each(function () {
                ...
                e = $(this),
                    ...
                    o = {
                        ...
                        maxFiles: t ? null : 1,
                        maxFilesize : e.data("allow-filesize"),
                        acceptedFiles: t ? null : "image/*",
                        paramName : e.data("paramname"),
                        headers: {
                        ...
                        init: function () {
                            ...
                            this.on("error", function(file, message) {
                                Swal({
                                    type : 'error',
                                    title : message,
                                    confirmButtonText :Swal.prototype.lang('ok'),
                                    confirmButtonClass : 'btn btn-success',
                                });
                                //this.removeFile(file);
                            });
                            this.on("success", function(file, response) {
                                $(e.data("target")).val(response.data[0].id);
                            });
                            ...
                            if(e.data("image")){
                                ...
                                this.createThumbnailFromUrl(mockFile, e.data("image"), function() {
    ...
    Datepicker = function () {
        ...
        if ( e.length) {
            $.getScript(`${location.origin}/vendor/bootstrap-datepicker/dist/js/bootstrap-datepicker.min.js`).done(()=>{
                if ($('html').attr('lang') != 'en') {
                    $.getScript(`${location.origin}/vendor/bootstrap-datepicker/dist/locales/${$('html').attr('lang')}.js`).done(()=>{
                       fn();
                    });
                }else{
                   fn();
        ...
        if ($(".input-slider-container")[0] && $(".input-slider-container").each(function () {
            ...
            noUiSlider.create(e, {
                ...
                e.noUiSlider.on("update", function (e, a) {
                    var min = $(this.target).data('target-min');
                    var max = $(this.target).data('target-max');
                    if (a == 0) {
                        $(`[name=${min}]`).val(e[a]);
                    }else{
                        $(`[name=${max}]`).val(e[a]);
        ...
        e.length && e.each(function () {
            ...
            e = $(this),
                ...
                quill.on('text-change', function() {
                    if($(quill.root).text()){
                        $(e.data('target')).val(quill.root.innerHTML);
                    }else{
                        $(e.data('target')).val('');
                    }
    ...
    Tags = function () {
        ...
        e.length && e.each(function () {
            ...
            var id = $(this).attr('id');
            $(`[for="${id}"]`).click(()=>{
                $(this).data('tagsinput').$input.focus();
            })
            $(this).parent().click(()=>{
                $(this).data('tagsinput').$input.focus();
            })
